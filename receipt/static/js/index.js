...
function additem(){
  ...
  if (order_list.includes(product_name)){
    tjtAlert('This product has already been selected! Please update in the summary!', invoice_container);
  }else{

    // check for empty fields
    if (quantity === ''){
      tjtAlert('quantity field is empty', invoice_container);
    }else if (quantity <= 0){
      tjtAlert('please select a value greater than 0', invoice_container)
    }else if (product_name ==='0'){
      tjtAlert('Select a product!', invoice_container)
    }else if (discount === '0'){
      tjtAlert('Select discount', invoice_container)
    }else{
...
function updateSummary(button){
  ...
  if (button.dataset.action == 'add'){
    ...
    if (quantity_el_value != 1){
      ...
      tjtAlert('click the bin icon to delete product from list', invoice_container)
    }
  }    
...
includeFee.addEventListener( 'click' ,(event) =>{
  ...
  if(state == '0'){
    if (delivery_fee != ''){
      ...
      tjtAlert("Please insert a delivery fee in order to use this function", invoice_container)
